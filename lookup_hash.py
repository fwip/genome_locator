...
class LookupHash(object):
    ...
    def keys(self):
        return [i for i in range(len(self.count)) if self.counts[i] != 0]
    ...
    def import_table(self, table):
        ...
        max_count = max(len(v) for v in table.values())
        ...
        #occupancy =  1.0 * list_len / array_len
        ...
        if occupancy < 0.25:
            ...
            self.offsets = LookupHash.init_array(array_len, max_count)
            self.counts = LookupHash.init_array(array_len, max_count)
